...
| $\chi_L$ | $1.509\times10^{-10}$ m | 定理 1.1.5.01（谱展开/Born法则） | 长度标度 |
| $\chi_T$ | $3.616\times10^{-17}$ s | 定理 1.1.5.01（谱展开/Born法则） | 时间标度 |
| $\Lambda$ | 3 | 互锁常数 | 三等分 |
| $k_0$ | 2 | 互锁常数 | 二分紧致性 |
...
> **诚实标注（n=22的双重推导）**：$n=22$ 在本文中出现两次——定理 9.1.2.06通过陈类-辛体积-谱展开/Born法则路径导出，定理 9.1.9.05通过谱流-角度周期边界路径导出。两条路径给出相同数值22，但其等价性依赖辛几何与 $\mathcal{MCE}$ 三扇区的深层对偶，目前标注为**待证明的框架内等价性**，非循环论证。
...
> **诚实标注**：$\Xi_E$ 的数值在YBCO示范计算中存在内部矛盾。本文已在附录B中修正。$\Xi_E$ 的形式定义，其具体数值依赖于测试函数 $a$ 的选择。在Tc公式中，$\Xi_E$ 与 $K$ 和 $\Psi_m$ 联合出现，其净效应由体材料Pb的 $T_c=7.2$ K反推自洽性验证。
...
> **诚实标注（公式-数值矛盾）**：当前公式 $\cot(5.91^\circ)/(\sqrt{2}\cdot(391.05)^{1/4})$ 直接代入计算得 $1.536$，与声称的 $0.1729$ 不符。下游 $\delta_0(7)=\delta_0^{\text{single}}\cdot\sqrt{7}=0.4575$ 与 $\delta_0^{\text{single}}=0.1729$ 自洽，表明数值 $0.1729$ 是正确的但公式转录有误。公式需作者审查修正。
...
$$k_B T_c^{\text{topo}} = \frac{\sqrt{\lambda_1}\cdot Q_{\text{geo}}\cdot g_{\text{pair}}^{\text{topo}}\cdot\delta_0^2}{\ln\Omega_0^{\text{topo}}}, \quad g_{\text{pair}}^{\text{topo}} = \sqrt{\frac{2(l1)^2}{3}} \cdot \sin\theta_I$$
