...
- `[260720.2]` **共扼谱几何与弱混合角新术语批量登记**：两篇 JMP 投稿文章（「共扼谱几何：从数学到物理的演化」「弱混合角的几何起源」）共引入约 20 个新术语，涉及 §4 定理（7 条：共扼定理三合一、δ⁸ 回路 Berry 相位定理、圆满性判据、乘子序列唯一性定理、代数约束定理、弱混合角几何定理）、§5 常数（5 条：前置因子比 κ_w/κ_w'、编码基态 N₁、终端值 N₇、景观范围、屏蔽因子）、§8 概念（9 条：零之动、自洽性筛选、凝聚相三角构型、编码轨道、预算-基因型双层、Birkhoff 混合矩阵、法丛 Spin(8)/S⁷、信息场自反馈屏蔽、扇区投影正弦对应）、§9 构造性概念（4 条：精细结构常数景观、观测者自举 CSG 版本、谱间隙公式、代数作用量 S(σ)）。→ 各节已按语义归属分别追加登记。无术语冲突（共扼谱几何为独立新框架，弱混合角为从该框架出发的派生文章）。旧版 6 号文章「弱混合角」使用不同推导路线（附录 C.4 路线），新条目不与旧条目冲突——两条路线互补。新版术语表版本号同步更新为 260720.2。
