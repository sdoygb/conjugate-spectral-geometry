...
| λ₁^eff | 391.05 rad⁻² | 跨扇区耦合 | 有效谱间隙方向 |
| λ₂^eff | 59324.3 rad⁻² | 跨扇区耦合 | 有效Dirac谱方向 |
| χ_L | 1.509×10⁻¹⁰ m | 定理 1.1.5.01（谱展开/Born法则） | 长度标度 |
| χ_T | 3.616×10⁻¹⁷ s | 定理 1.1.5.01（谱展开/Born法则） | 时间标度 |
| Λ | 3 | 互锁常数 | 三等分 |
| k₀ | 2 | 互锁常数 | 二分紧致性 |
| Γ_geo | 5.75×10⁻²³ | 定理 3.2.2.01（信息场衰减定理） | 信息场几何衰减率 |
| τ_dec | ~7.28日 | 定理 3.2.2.01（信息场衰减定理） | 信息场退相干时间 |
| N_dec | 1.74×10²² | 定理 3.2.2.01（信息场衰减定理） | 退相干事件数 |
...
| 材料 | T_c^实验 (K) | ω_M^预言 (K) | Θ_D (K) | T_c/Θ_D |
|:---|:---:|:---:|:---:|:---:|
...
| T_c 公式 | 1.13\,Θ_D\,e⁻1/N(0)V | E_scale· g_pair·δ₀²/lnΩ₀ |
...
|:---|:---|:---|:---|
| **体材料** | 无 | — | **绝对严格** |
| **拓扑纳米线** | N（构造参数）、r_phys | 理论实验输入 | 给定后严格 |
| **铜基** | N（层数）、p（掺杂） | 材料制备 | pₒₚₜ 内生；给定后严格 |
| **铁基** | N_orb（轨道数）、η（晶格畸变） | 原子结构 | ε 已几何化 |
| **重费米子** | m^*（有效质量比） | 一阶定理实验 | 一阶严格；高阶几何级数 |
| **有机** | D_eff（有效维度） | 化学结构 | E_g、aₘₒₗ 已封闭 |
| **二维莫尔** | θ（转角）、N（层数） | 器件制备 | θ_magic 内生 |
...
| 类型 | 标签 | Σ_cond 拓扑 | N_eff | 超导可能 | 代表空间群 |
|:---:|:---|:---:|:---:|:---:|:---|
| **A** | Z₂可超导 | \tilde{K}⁰=ℤ^N_eff-1 | 3,5,7 | ✅ | R\bar{3}m(166), Fd\bar{3}m(227) |
| **B** | 镜面对称可超导 | \tilde{K}⁰=ℤ^N_eff-1 | 5 | ✅ | Fm\bar{3}m(225), Pm\bar{3}m(221) |
| **C** | 非中心对称条件可超导 | \tilde{K}⁰=ℤ^N_eff-1(条件) | 3 | 🟡条件性 | F\bar{4}3m(216), I\bar{4}3m(217) |
| **D** | 磁性可超导（需修正） | \tilde{K}⁰≠0(PT保护) | 3-7 | 🟡修正后 | R\bar{3}c(167), P4/nmm(129,反铁磁) |
| **E** | 不可超导 | \tilde{K}⁰=0 | 1 | ❌ | 其他全部 |

**A类（Z₂可超导，约20个空间群）**：

| 晶系 | 空间群 | N_eff | 已知材料 | T_c范围(K) |
|:---|:---:|:---:|:---|:---:|
...
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|
| Bi₂Se₃纳米线 | R\bar{3}m(166) | Z₂强TI | ✅ | ✅ N_eff=7 | ✅ | 🟢超导 | 438(预测) |
| SnTe纳米线 | Fm\bar{3}m(225) | TCI镜面 | ✅ | ✅ N_eff=5 | ✅ | 🟢超导 | 228(预测) |
| YPtBi纳米线 | F\bar{4}3m(216) | 非中心对称 | ✅ | ✅ N_eff=3 | ✅ | 🟢超导 | 21(预测) |
...
| R\bar{3}m | 166 | D₃d | 候选 | Bi₂Se₃族 | 276-438 |
...
| 参数 | 数值 | 来源 |
|:---|:---|:---|
| v_F | 5.07×10⁵ m/s | 定理 9.1.5.02（指标定理） |
| r_phys | 10 nm | 实验可调参数 |
| r_geo | r_phys/χ_L = 66.3 | 谱展开/Born法则映射 |
| N | 7 | 构造参数 |
| g_pair | 4.282 | \sinθ_C^e/\sinθ_I^e |
| δ₀(7) | 0.4575 | δ₀^single·√(7) |
...
| 化合物 | v_F (10⁵ m/s) | Z₂确认 | ARPES确认 | 体T_c (K) | 三螺旋T_c (K) | 置信度 |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|
...
| 编号 | 定理 | 内容 | 状态 |
|:---:|:---|:---|:---:|
| T1 | v_F 严格公式 | v_F = c·(λ₁^eff)³/2/(S_e²·√(λ₂^eff)) | ✅ 定理级 |
| T2 | Bott周期性七层截断 | N_eff\le7 由三重封印锁定 | ✅ 定理级 |
| T3 | d_IR 的K-理论解释 | d_IR = rank(\tilde{K}⁰) + 1 | ✅ 定理级 |
| T4 | δ₀(N) 标度律 | δ₀(N) = δ₀^single·√(N_eff) | ✅ 定理级 |
| T5 | lnΩ₀ 双路径锁定 | HRRRMT双重验证 | ✅ 定理级 |
| T6 | Tc公式熵稀释推导 | k_B T_c = E_scale· g_pair·δ₀²/lnΩ₀ | ✅ 定理级 |
| T7 | g_pair=4.282 | \sinθ_C^e/\sinθ_I^e | ✅ 严格 |
...
|:---:|:---|:---:|:---|:---|
| C1 | 体材料 Δ S_pair 的几何理论 | 15=dim\mathfrak{spin}(6) 依赖选群唯一性 | 体材料路径 | 完成辛约化选群唯一性论证 |
| C2 | 温度谱展开/Born法则 Ξ_T 的独立计算 | 热核系数比依赖测试函数选择 | 理论自洽性闭环 | zeta函数正则化路径 |
| C3 | Eₜₒₚₒ 中 \sin³θ_M 幂次唯一性 | 层级分辨率确定但非唯一 | T_c 变动±15-18% | 谱展开/Born法则层级乘子序列严格推导 |
...
| 项目 | 地位 | 说明 |
|:---|:---|:---|
| S_e, λ₁, λ₂, K, χ_L, χ_T, Λ, k₀ | **纯几何输出** | 框架锁定 |
| Γ_geo, τ_dec, N_dec | **纯几何输出** | 锁定 |
| θ_M⁰, θ_C⁰, θ_I⁰ | **纯几何输出** | 电子基态构型 |
| ε_pair^norm=15·(λ₁/λ₂)· S_e⁻¹ | **几何构造**（给定ansatz） | 15依赖 \mathfrak{spin}(6) 选择 |
...
| 2 | K-理论 | 定理 9.1.3.01：d_IR=K-群秩 | 全部 | 严格 |
| 3 | K-理论 | 定理 9.1.3.02：N_eff\le7=Bott周期 | 全部 | 严格 |
| 4 | 非交换几何 | 定理 9.1.4.02：Ξ_E=Dixmier迹 | 全部 | 严格（形式） |
| 5 | 指标定理 | 定理 9.1.5.02：v_F=指标体积 | 拓扑纳米线 | 严格 |
| 6 | 层论 | 定理 9.1.6.01：lnΩ₀=Euler示性数 | 全部 | 严格 |
| 7 | 层论Clifford | 定理 9.1.6.03：ε_pair^norm | 全部 | 严格（给定ansatz） |
| 8 | RMT | 定理 9.1.7.02：lnΩ₀=谱熵 | 全部 | 严格 |
| 9 | 谱理论 | 定理 9.1.9.05：pₒₚₜ=22/S_e | 铜基 | 严格 |
...
| ℐ扇区{e₇,e₈,e₉} | 信息界标架 | 定理 9.1.3.02, 9.1 | Bott周期截断 N_eff\le7 |
| 一阶条件 [[𝒟,a],Jb^*J⁻¹]=0 | 𝒞⊕ℐ 扇区互扼 | 定理 9.1.3.01, 15.1 | λ₂/λ₁≈150 谱刚性 |
| 谱间隙方向分离 λ₁^eff | ℳ 扇区独立 | 定理 7.14.2.01, 10.2 | 面内谱间隙方向=λ₁/N_eff |
| Dirac谱方向分离 λ₂^eff | 𝒞⊕ℐ 扇区联合 | 定理 7.14.2.02, 9.4 | 层间刚度=λ₂\sin²θ_I/N_eff |
