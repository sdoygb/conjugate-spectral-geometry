...
| 圈 | 恒等式 | 验证 | 不可约项 |
|:--|:--|:--|:--|
| **第一圈（线性）** | Λ + k₀ - ΔΘ = 0 | 3+2-5=0 ✓ | 无 |
| **第二圈（二次）** | Λ² - k₀² - ΔΘ = 0 | 9-4-5=0 ✓ | Λ² = 9 |
| **第三圈（三次）** | Λ³ - ΔΘ² - k₀ = 0 | 27-25-2=0 ✓ | Λ³ = 27 |
| **第四圈（交叉）** | Λ × ΔΘ = 15（破缺的 Sₜₒₜₐₗ=0） | 3 × 5 = 15 ✓ | Λ × ΔΘ = 15 |
...
| 项 | 圈 | 公式 | 值 | 量级 | 状态 |
|:--|:--|:--|:--|:--|:--|
| B₀ | 第二圈 | 2⁷ + Λ² | 137 | 10² | 定理（分支内） |
| B₁ | 第三圈 | 1/Λ³ | 1/27 ≈ 0.037037 | 10⁻² | 定理（分支内） |
| B₂ | 第四圈 | -Λ × ΔΘ / 14400 | -15/14400 ≈ -0.0010417 | 10⁻³ | 定理（分支内） |
| B₃ | 干涉 | (Λ/k₀)/(Λ³ · 14400) | 1/259200 ≈ 3.86× 10⁻⁶ | 10⁻⁶ | 定理（分支内） |
| B₄ | 残留回声 | -ΔΘ · (Λ/k₀)²/14400² | -1/18432000 ≈ -5.43× 10⁻⁸ | 10⁻⁸ | 定理（分支内） |
| B₅ | 二阶回声 | B₂/14400 | -1/13824000 ≈ -7.23× 10⁻⁸ | 10⁻⁸ | 定理（分支内） |
| B₆ | 三阶回声 | B₃/14400 | ∼ 2.7 × 10⁻¹⁰ | 10⁻¹⁰ | 量级已定，精确值待定 |
...
| Morel 等 2020（原子干涉法） | 137.035 999 046 | -5.6 × 10⁻⁸ |
...
| 分叉口 | 类型 | 自由度 |
|:--|:--|:--|
| **分叉口 1：Triality 破缺 S₃ → Z₂** | 真正的分支选择 | 三个等价的 Z₂ 子群 |
| **分叉口 2：σ₀ 精确值** | 参数简并 | 可能唯一或有限简并 |
| **分叉口 3：观测者位置 T** | 参数偏移 | 同一宇宙，不同观测点 |
| **前分叉口：δ → \{2,3,5\}** | 几何必然 | **无自由** |
...
| E₈ 唯一 | Minkowski–Serre（数学定理） |
| h = 30 | 标准李理论（240/8） |
| \{2,3,5\} | 唯一素因子分解 30 = 2 × 3 × 5 + 加性约束 2+3=5 |
...
| 分支 | p | B₀ | α⁻¹ ≈ | 稳定扇区 |
|:--|:--|:--|:--|:--|
| Z₂⁽ᵛ⁾ | 3 | 137 | 137.036 | ℳ（物质） |
| Z₂⁽ˢ⁾ | 2 | 132 | 132.1 | 𝒞（因果） |
| Z₂⁽ᶜ⁾ | 5 | 153 | 153.0 | ℐ（信息） |
