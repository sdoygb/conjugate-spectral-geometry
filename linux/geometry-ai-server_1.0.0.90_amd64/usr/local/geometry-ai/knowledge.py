...
class VectorKnowledgeBase:
    ...
    def __init__(self, persist_dir: str):
        ...
        if self.embedding_fn is None:
            ...
            raise RuntimeError("[VECTOR] embedding_fn 不能为 None（强制 1024 维 SiliconFlow），请检查 API Key")
    ...
    def initialize(self) -> bool:
        ...
        # Embedding 预检查（失败不阻塞，只记录警告）
        if self.embedding_fn is not None:
            try:
                test = self.embedding_fn(["启动测试"])
                if not test or not test[0]:
                    logger.warning("[VECTOR] embedding function 返回空结果，仍尝试初始化 ChromaDB")
            except Exception as e:
                logger.warning(f"[VECTOR] embedding function 预检查失败（不阻塞）: {e}")
